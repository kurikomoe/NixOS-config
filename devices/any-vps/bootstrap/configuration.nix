...
p @ {
  ...
  #!FIXME Change this according to vdisk on vps
  # Notes:
  # Linode: /dev/sda
  vpsDisk = "/dev/sda";
in {
  ...
  boot.loader.grub = {
    configurationLimit = 3; #!FIXME avoid using up the /boot disk space
    efiSupport = true;
    devices = [vpsDisk];
  ...
  networking.hostName = lib.mkDefault "KurikoLinode";

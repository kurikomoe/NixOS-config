...
runKexec() {
  ...
  if [[ $kexecUrl == "" ]]; then
    case "${isArch}" in
    x86_64 | aarch64)
      kexecUrl="https://gh-proxy.com/https://github.com/nix-community/nixos-images/releases/download/nixos-24.11/nixos-kexec-installer-noninteractive-${isArch}-linux.tar.gz"
      ;;
    *)
...
SSH
  ...
  if [[ ${hasIpv6Only} == "y" ]] && [[ $kexecUrl == "https://gh-proxy.com/https://github.com/"* ]]; then
